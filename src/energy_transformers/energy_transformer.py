...
class EnergyHead(nn.Module):
    ...
    def energy(self, x):
        ...
        if hasattr(self, "tril"):
            ...
            logsumexp_wei = logsumexp_wei.masked_fill(
                all_mask_rows.squeeze(-1), 0.0
            )
...
class MultiHeadEnergyAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.heads = nn.ModuleList(
            [EnergyHead(config) for _ in range(config.n_head)]
        )
        self.dropout = nn.Dropout(config.dropout)
...
class EnergyTransformerBlock(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        x = x - self.proj(
            self.attn(self.ln(x)) + self.scale_ff * self.ffwd(self.ln(x))
        )
        return x
